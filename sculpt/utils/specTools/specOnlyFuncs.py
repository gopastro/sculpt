...
if __name__ == "__main__":
    ...
    if maximaList == []:
        ...
        x_apml = float(raw_input('Enter the amplitude in range:('+str(min(testSpec))+','+str(max(testSpec))+')'))
    else:
        (x_mean,x_apml) = maximaList[0]
    g_first = models.Gaussian1D(x_apml, x_mean, 5)
    fitter = fitting.SLSQPLSQFitter()
    g_firstFit = fitter(g_first,x,testSpec)
    fit_info = fitter.fit_info
    R_val = fit_info['final_func_val']
    first_Rvalue = R_val
    R_val_current = R_val
    if showFitterPlots: testSpecPlotting(testSpec,x,g_firstFit,g_first, showPlot=True)
    ...
    if 1 < len_maximaList:
        for loopIndex in range(1,len_maximaList):
            ...
            if showFitterPlots: testSpecPlotting(testSpec,x,gg_Fit,gg_init, showPlot=True)
